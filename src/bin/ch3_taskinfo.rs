...
use user_lib::{
    get_time, println, task_info, TaskInfo, TaskStatus, SYSCALL_EXIT, SYSCALL_GETTIMEOFDAY,
...
pub fn main() -> usize {
    ...
    assert_eq!(3, info.syscall_times[SYSCALL_GETTIMEOFDAY]);
    assert_eq!(1, info.syscall_times[SYSCALL_TASK_INFO]);
    assert_eq!(0, info.syscall_times[SYSCALL_WRITE]);
    assert_eq!(0, info.syscall_times[SYSCALL_YIELD]);
    ...
    assert_eq!(5, info.syscall_times[SYSCALL_GETTIMEOFDAY]);
